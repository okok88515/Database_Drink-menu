...
public class L extends DrinkingShop{
	/*
	 * L:Constructor for TeaShop,Farmer
	 */
	public L(String name, String telephone, String breakDay, String operation, String address, Statement statement) {
	...
	public String teaInfo(String tea) throws SQLException {
		...
		try {
			...
			message += String.format("%20s%30s%10s\n", "品項", "英文", "大杯價格");
			while (result.next()) {
				message += String.format("%20s%30s%10d\n", result.getString("品項"), result.getString("English"),
						result.getInt("大杯價格"));
			}
			message += ("-".repeat(100) + "\n");
			result.close();
